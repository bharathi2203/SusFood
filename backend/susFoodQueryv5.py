...
class Map:
    ...
    def showMap(self):
        ...
        g = geocoder.ip("me")
        # myAdd = g.latlang
        # folium.CircleMarker(location = myAdd, radius = 20, popup="ME").add_to(my_map)
        folium.Marker(location=coords, popup="ME", tooltip = "ME", icon = folium.Icon(icon = 'user', icon_color= 'white', color = 
        'black')).add_to(my_map)
        #Display the map
        ...
        folium.Circle(location=coords, popup='Point 1A', fill_color='green', radius=300, weight=2, color="green").add_to(my_map)
